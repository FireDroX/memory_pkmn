const statusKeys = new Map([
  ...
  ["Inscription impossible. Verifie ton pseudo et ton mot de passe.", "status.registerInvalid"],
  ...
  ["Trop de tentatives de connexion. Reessaie dans une minute.", "status.loginRateLimited"],
  ["Trop de tentatives, reessaie plus tard.", "status.rateLimited"],
  ["Trop de salons crees, reessaie dans une minute.", "status.inviteRateLimited"],
  ["Verification anti-robot requise.", "status.turnstileRequired"],
  ["Verification anti-robot invalide.", "status.turnstileInvalid"],
  ["Verification anti-robot indisponible.", "status.turnstileUnavailable"],
...
const statusPatterns = [
  ...
  {
    pattern: /^Salon (ROOM-[0-9a-fA-F-]+) cree\. Les invitations sont pretes !$/,
    key: "status.roomCreated",
    values: (match) => ({ roomID: match[1] }),
  },
  {
    pattern: /^Salon (ROOM-[0-9a-fA-F-]+) supprime\.$/,
